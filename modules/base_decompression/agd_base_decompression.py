...
def run(args):
  ...
  if 'reference' not in dataset_params:
  ...
  ref_index = []
  ref_seqs = []
  for contig in dataset_params['reference_contigs']:
    ref_index.append(contig['index'])
  ...
  output = persona_ops.agd_base_decompression(chunk_names=names, chunk_size=size, unpack=args.unpack, ref_sequences=ref_seqs,ref_index=ref_index,
